...
## Delegated ingest, connected read, and execution are separate authorities

ALVIRA now distinguishes three different relationships that must not collapse into one permission:

1. **Connected read** — a destination may read an approved Context view.
2. **Delegated Context ingest** — an explicitly delegated agent may contribute Context on the subject's behalf with actor/subject provenance and uncertainty preserved.
3. **Execution** — Agent OS or another harness may perform a bounded user-approved job under its own authorization and governance.

None implies either of the others. A connected AI cannot write Context because it can read it. An agent that contributed Context cannot execute arbitrary work because it was delegated to answer an interview. A paid execution entitlement does not authorize every action.

## Evidence → Reflect, never silent mutation

Execution can reveal stale, contradicted, ambiguous, missing, or unnecessary context. That evidence returns to ALVIRA as an observation/candidate, not as an automatic durable-memory update. Preserve source/actor provenance and uncertainty so execution-derived observations cannot be mistaken for subject-confirmed Context.
