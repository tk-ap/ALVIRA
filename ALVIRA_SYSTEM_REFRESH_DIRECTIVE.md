...
## Canonical Context Onboarding Model

All onboarding, interview, import, continuation, and context-seeding work must preserve `docs/ALVIRA_CONTEXT_ONBOARDING_MODEL.md` as the canonical onboarding model.

Its core rule is:

> **Seed what is already known, distinguish fact from inference, validate uncertainty, and ask only for genuine gaps.**

This includes a first-class **Bring what your AI already knows** path alongside **Start fresh**. Existing AI context profiles, conversation exports, documents, URLs, prior ALVIRA state, and other supported sources enter as provenance-aware seed evidence rather than unquestioned truth. Seeded context must be normalized, deduplicated, checked for contradiction/staleness, reviewed where material, and consumed by the adaptive interview before new questions are generated.

Do not create a parallel onboarding system for AI imports. Extend the existing shared interview/context-review lifecycle so all entry paths converge on one durable ALVIRA Context model.
...
1. **Build context** — through conversation, documents, files, URLs, AI context imports, and other sources.
...
The stronger canonical onboarding rule is defined in `docs/ALVIRA_CONTEXT_ONBOARDING_MODEL.md`: imported context—including context generated by another AI tool—is seed evidence with provenance and uncertainty, not automatic truth. The interview should consume this seed before generating questions and should focus validation on genuine gaps, contradictions, stale claims, and material inference.
...
2. Read `docs/ALVIRA_CONTEXT_ONBOARDING_MODEL.md` before changing onboarding, interviews, context imports, continuation, claim review, or seed handling.
3. Treat the branch as exploratory/preview-only.
4. Keep `main` and production untouched unless explicitly instructed otherwise.
5. Prefer reusable design tokens/components over one-off page styling.
6. Separate visual/brand changes from functional changes whenever possible.
7. Preserve existing routes and product behavior.
8. Verify protected flows after changes that touch shared layouts/components.
9. Avoid making unsupported capability claims in public copy.
10. Clearly distinguish demonstration/simulated states from live product intelligence when relevant.
11. Treat scroll-triggered behavior as a first-class UI requirement and provide reduced-motion/static fallbacks.
12. Optimize for a coherent product category and experience rather than cosmetic novelty.
13. Preserve the two-entry fluency architecture as two framing paths into one ALVIRA Context product; do not create separate beginner and advanced offerings.
14. Allow experienced users to bypass foundational AI education and reach portability/continuity/architecture value quickly.
15. Preserve one onboarding/context model across Start fresh, AI-context import, documents/files/URLs, and prior-context continuation; do not create parallel source-specific profile systems.
...
- That understanding is built from multiple context sources, including context the user has already developed with other AI tools.
- Imported context is normalized and validated rather than blindly treated as truth.
