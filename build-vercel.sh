...
echo "[1/5] install + Dossier naming guard"
...
bun install
bun test tests/dossier-naming.test.ts

echo "[2/5] vite build (light — safe under the sandbox memory cap)"
bun run build

echo "[3/5] schema gate: migrate + verify (fail deploy if schema can't match code)"
