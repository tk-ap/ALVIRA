...
1. A user completes an initial ALVIRA interview.
2. ALVIRA creates and maintains the user's living Context.
3. The user chooses Connect ALVIRA.
4. The user selects an AI tool or compatible destination.
5. ALVIRA shows what Context will be shared and asks for approval.
6. The destination can retrieve the approved Context when relevant.
7. When the user updates ALVIRA, connected destinations receive the current approved Context on subsequent reads.
8. The user can inspect and revoke every connection from ALVIRA.
...
1. complete an ALVIRA interview;
2. save living Context;
3. choose Connect ALVIRA;
4. select a supported AI destination;
5. understand and approve what will be shared;
6. use that AI with ALVIRA Context without manual prompt/file transfer;
7. update Context once in ALVIRA and have later connected reads reflect the maintained version;
8. revoke the destination from ALVIRA.
