...
## Contributor provenance and write boundary

Connect/Bridge is a distribution surface, not implicit Context-authoring authority.

- A destination that can read an approved Context view cannot write to canonical Context merely because it is connected.
- If an AI/agent is separately commissioned to contribute Context on the subject's behalf, that occurs through an explicit delegated-ingest mode with actor/subject separation and persisted provenance.
- Agent-contributed claims must retain their material epistemic state: KNOWN-by-agent, INFERRED, or UNKNOWN.
- KNOWN-by-agent must not be rewritten as subject-confirmed truth unless the subject actually confirms it.
- Approved Context projections should preserve contributor provenance or uncertainty whenever dropping it would materially change the meaning of the Context.
- Any future agent write-back or Context-update proposal remains subject to ALVIRA's explicit review/authorization rules; Bridge read scopes do not grant that authority.
