...
To deploy code changes, use only `scripts/e2e/e2e-env deploy`. It refuses an unlinked folder, a dirty/uncommitted worktree, a non-test database, an E2E overlay whose recorded canonical-sandbox baseline is behind the remote `sandbox/review` head, or a bundle that is not E2E-stamped. Before building, it checks the current E2E alias release fingerprint and health; when the E2E commit and canonical-sandbox SHA already match a healthy alias, it **reuses the existing deployment instead of creating another Vercel deployment**. After a real deploy it stamps `/e2e-release.json`, aliases the preview and checks health.
