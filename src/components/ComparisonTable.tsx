...
export function ComparisonTable({ competitors = COMPARISON_COMPETITORS, dimensions = COMPARISON_DIMENSIONS, highlightRow }: Props) {
  ...
  return <div className="mt-10">
    <div className="hidden overflow-x-auto sm:block">
      <table className="w-full min-w-[52rem] border-collapse text-left">
        ...
        <thead><tr className="border-b-2 border-gray-200 dark:border-gray-700">
          ...
          {cols.map((competitor, index) => <th scope="col" key={competitor.id} className={`px-3 py-3 font-mono text-xs uppercase tracking-wide ${index === 0 ? "bg-system-soft/70 text-system-dark dark:bg-ink/30 dark:text-system" : "text-gray-500 dark:text-gray-400"}`}>{competitor.featureName ?? competitor.displayName}</th>)}
        </tr></thead>
        <tbody className="text-sm leading-relaxed">{rows.map((row, rowIndex) => <tr key={row.id} className={`border-b border-gray-100 dark:border-gray-800 ${highlightRow === rowIndex ? "bg-system-soft/40 dark:bg-ink/20" : ""}`}>
          <th scope="row" className="py-3.5 pr-4 font-semibold text-gray-900 dark:text-gray-100">{row.label}</th>
          {cols.map((competitor, index) => { const cell = row.cells[competitor.id]; return <td key={competitor.id} className={`px-3 py-3.5 ${index === 0 ? "bg-system-soft/40 dark:bg-ink/20" : ""}`}><span role="img" aria-label={cellLabel(cell.mark, cell.note)}>{MARKS[cell.mark]}</span>{cell.note ? <span className="ml-1 text-gray-600 dark:text-gray-400">{cell.note}</span> : null}</td>; })}
        </tr>)}</tbody>
      </table>
    </div>
    <div className="space-y-6 sm:hidden">{rows.map((row, rowIndex) => <div key={row.id} className="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
      <div className="border-b border-system bg-system-soft px-5 py-3 font-mono text-xs uppercase tracking-wide text-system-dark dark:border-system-dark dark:bg-ink/40 dark:text-system">{row.shortLabel}</div>
      {cols.map((competitor, index) => { const cell = row.cells[competitor.id]; return <div key={competitor.id} className={`flex items-start gap-2 border-b border-gray-100 px-5 py-3 text-sm last:border-b-0 dark:border-gray-800 ${index === 0 ? "bg-system-soft/30 dark:bg-ink/10" : ""}`}><span role="img" aria-label={cellLabel(cell.mark, cell.note)} aria-hidden="false">{MARKS[cell.mark]}</span><span className="font-medium text-gray-900 dark:text-gray-100">{competitor.featureName ?? competitor.displayName}:</span><span className="text-gray-600 dark:text-gray-400">{MARKS_LABEL[cell.mark]}{cell.note ? ` — ${cell.note}` : ""}</span></div>; })}
    </div>)}</div>
