...
import { getOwnerDashboardDepth, type OwnerDashboardDepth as Depth } from "~/routes/-ownerDashboardDepth";
...
export function OwnerDashboardDepth() {
  ...
  useEffect(() => {
    ...
    getOwnerDashboardDepth().then(setData).catch(() => setDenied(true));
          ...
          {data.interventions.length === 0 ? (
            ...
            <div className="mt-5 space-y-2">
              {data.interventions.map((item) => (
                <div key={`${item.email}-${item.reason}`} className="grid gap-2 border-t border-human/20 pt-3 sm:grid-cols-[1fr_1.4fr_auto] sm:items-center">
