...
export function PortabilityGraphic() {
  const tools = [
    { label: "ChatGPT", x: 280, y: 46 },
    { label: "Claude", x: 466, y: 108 },
    { label: "Gemini", x: 420, y: 227 },
    { label: "Cursor", x: 140, y: 227 },
    { label: "Your Stack", x: 94, y: 108 },
  ];
  return (
    <svg
      viewBox="0 0 560 300"
      ...
      role="img"
      aria-label="ALVIRA knowledge files connect to ChatGPT, Claude, Gemini, Cursor, and your stack."
    >
      ...
      <text
        x="24"
        y="28"
        className="font-mono text-warm-gray/90"
        fontSize="10"
        letterSpacing=".12em"
        fill="currentColor"
      >
      ...
      >
        {tools.map((tool, i) => (
          <path
            key={tool.label}
            d={`M280 150 L${tool.x} ${tool.y}`}
            pathLength="1"
            className="pg-line text-system/45"
            style={{ animationDelay: `${0.35 + i * 0.1}s` }}
          />
        ))}
      </g>
      <g className="pg-fade" style={{ animationDelay: ".3s" }}>
        <circle
          cx="280"
          cy="150"
          r="48"
        ...
        />
        <path
          d="M264 139h32v25h-32zM270 134h20v5h-20zM272 146h16m-16 6h12"
          fill="none"
          stroke="var(--color-system)"
          strokeWidth="1.2"
          strokeLinecap="round"
