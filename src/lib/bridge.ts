...
const BRIDGE_PROPOSE_SCOPE = "context:read profile:read context:propose";
...
export function resolveBridgeScope(requestedScope: string | null | undefined) {
  const requested = requestedScope?.trim();
  if (!requested) return BRIDGE_READ_SCOPE;
  ...
  return scopes.includes("context:propose") ? BRIDGE_PROPOSE_SCOPE : BRIDGE_READ_SCOPE;
}
...
export async function issueBridgeAuthorizationCode(
  ...
  await getDb().query(
    ...
    [hashBridgeSecret(code), userId, clientId, redirectUri, selectedProfileId, destination, options.codeChallenge || null, options.codeChallengeMethod || null, resolveBridgeScope(options.requestedScope), expiresAt],
  );
...
export async function exchangeBridgeAuthorizationCode(
  ...
  const grantedScope = row.scope || BRIDGE_READ_SCOPE;
  await db.query(
