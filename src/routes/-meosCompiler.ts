...
export interface MeosPortrait {
  ...
  sourceConfidence: {
    userSupplied: string[];
    inferred: string[];
    selfValidated: string[];
  };
}

const emptyPortrait = (): MeosPortrait => ({
  portrait:
    "Your portrait will take shape as you share more about your current chapter, values, and direction.",
  purposeStatements: { personal: "", professional: "" },
  decisionCompass:
    "When facing a decision, ask yourself what aligns with your values and boundaries.",
  dailyAlignment:
    "Take a moment to notice what matters today and choose one action that supports it.",
  cycles: "Your current chapter is still unfolding.",
...
export async function generatePortrait(
...
): Promise<MeosPortrait | { error: string }> {
  ...
  try {
    ...
    return {
      ...
      ...parsed,
      purposeStatements: {
        ...emptyPortrait().purposeStatements,
        ...(parsed.purposeStatements || {}),
      },
      sourceConfidence: {
        ...emptyPortrait().sourceConfidence,
...
export function compileMeosKnowledge(
      ...
      ? section(
          ...
          birthValues.map((v) => {
            try {
              ...
              return `Natal chart data recorded: ${d.date || "date not supplied"}, ${d.unknownTime ? "time unknown" : d.time || "time not supplied"}, ${d.location || "location not supplied"}`;
            } catch {
              return v;
            }
          }),
  ...
  const allFiles = {
    ...
    "purpose-statements.md": purposeStatements,
    "boundaries.md": boundaries,
    "definition-of-success.md": definitionOfSuccess,
    "decision-compass.md": decisionCompass,
    "career-tools.md": careerTools,
    "cycles.md": cycles,
    "source-trace.md": sourceTrace,
  };
  return {
    portrait,
    purposeStatements,
