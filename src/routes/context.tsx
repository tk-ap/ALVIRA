      ...
      <main id="main-content">
        <section className="border-b border-gray-200 px-6 py-20 dark:border-gray-800 sm:px-8 sm:py-28">
          <div className="mx-auto max-w-5xl">
            ...
            <h1 className="mt-5 max-w-3xl text-4xl font-bold leading-tight tracking-tight sm:text-6xl">Start with what already exists.</h1>
            <p className="mt-6 max-w-2xl text-lg leading-relaxed text-gray-600 dark:text-gray-400">Your context is already spread across the internet, your files, and other AI tools. Bring the sources you want ALVIRA to understand. It will use them as evidence, then interview you about what is missing or unclear.</p>
          </div>
        </section>
          ...
          <div className="mx-auto grid max-w-5xl gap-10 lg:grid-cols-[1.1fr_.9fr]">
            <div className="grid gap-3 sm:grid-cols-2">
              {CONTEXT_SOURCE_OPTIONS.map((option) => {
                ...
                return (
                  <button key={option.type} type="button" onClick={() => setSelectedType(option.type)} className={`border p-5 text-left transition-colors ${active ? "border-system bg-system/5 dark:bg-system/10" : "border-gray-200 hover:border-gray-400 dark:border-gray-800 dark:hover:border-gray-600"}`} aria-pressed={active}>
                    <div className="flex items-start justify-between gap-3"><h2 className="font-semibold">{option.label}</h2>{active && <span className="font-mono text-xs text-system-dark dark:text-system">SELECTED</span>}</div>
            ...
            <div className="border border-gray-200 bg-gray-50 p-6 dark:border-gray-800 dark:bg-gray-900 sm:p-8">
              ...
              {selectedType === "file" || selectedType === "ai-context" ? (
                ...
                <a href="/interview" className="mt-7 inline-flex rounded-lg bg-gray-900 px-5 py-3 font-semibold text-white hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-gray-200">Start the interview →</a>
              ) : (
        ...
        <section className="border-y border-gray-200 bg-gray-950 px-6 py-16 text-white dark:border-gray-800 sm:px-8 sm:py-20">
          <div className="mx-auto max-w-5xl grid gap-8 md:grid-cols-3">
            {[["01 / OBSERVE", "ALVIRA extracts supported signals from the sources you choose."], ["02 / ASK", "It finds missing, conflicting, stale, or low-confidence context and asks targeted follow-ups."], ["03 / BUILD", "Confirmed understanding becomes reusable AI context you can carry to other tools."]].map(([title, body]) => (
              <div key={title} className="border border-gray-800 bg-gray-900 p-6"><p className="font-mono text-xs text-system">{title}</p><p className="mt-4 text-sm leading-relaxed text-gray-300">{body}</p></div>
            ))}
          </div>
        </section>
        ...
        <section className="px-6 py-16 sm:px-8 sm:py-20">
          <div className="mx-auto max-w-5xl">
            ...
            {sources.length === 0 ? (
              <div className="mt-8 border border-dashed border-gray-300 p-10 text-center dark:border-gray-700"><p className="font-mono text-sm text-gray-500 dark:text-gray-400">No sources yet.</p><p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Add a website, professional profile, social profile, or start with the interview.</p></div>
            ) : (
        ...
        <section className="border-t border-gray-200 bg-gray-50 px-6 py-16 dark:border-gray-800 dark:bg-gray-900 sm:px-8 sm:py-20">
          <div className="mx-auto max-w-5xl"><div className="max-w-3xl"><span className="font-mono text-xs uppercase tracking-wide text-system-dark dark:text-system">Trust model</span><h2 className="mt-3 text-2xl font-bold">ALVIRA does not assume that everything it finds is true.</h2><p className="mt-4 text-sm leading-relaxed text-gray-600 dark:text-gray-400">Every useful signal should be traceable to a source and separated from inference. User confirmation is what turns an observation into trusted context.</p></div><div className="mt-8 grid gap-3 sm:grid-cols-4">{[["Observed", "Found in a source"], ["Inferred", "Pattern suggested by evidence"], ["Confirmed", "You say it is true"], ["Outdated", "Previously true, no longer current"]].map(([title, body]) => <div key={title} className="border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-950"><p className="font-mono text-xs text-system-dark dark:text-system">{title}</p><p className="mt-2 text-sm text-gray-600 dark:text-gray-400">{body}</p></div>)}</div></div>
        </section>

        <section className="px-6 py-16 sm:px-8 sm:py-20"><div className="mx-auto max-w-3xl text-center"><h2 className="text-2xl font-bold sm:text-3xl">Ready to fill the gaps?</h2><p className="mt-4 text-gray-600 dark:text-gray-400">Sources give ALVIRA a head start. The interview turns uncertainty into understanding.</p><a href="/app" className="mt-7 inline-flex rounded-lg bg-gray-900 px-6 py-3.5 font-semibold text-white hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-gray-200">Continue to ALVIRA →</a></div></section>
      </main>
      <TrustFooter />
    </div>
  );
