...
function ProfileSection({ profiles, draft, remove, owner }: { profiles: Profile[]; draft: { offering: string; topic: string; updated_at: string } | null; remove: (id: string) => Promise<void>; owner: boolean }) {
  return <section className={owner ? "border-t border-gray-200 pt-8 dark:border-gray-700" : ""}>{owner && <h2 className="mb-4 font-mono text-sm font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Your profiles</h2>}
    ...
    <div className="mt-8 grid gap-4 md:grid-cols-2"><a href="/integrations" className="border border-emerald-300 bg-emerald-50 p-5 transition-colors hover:border-emerald-500 dark:border-emerald-800 dark:bg-emerald-950/30"><p className="font-mono text-xs uppercase tracking-wide text-emerald-700 dark:text-emerald-400">AI integrations · beta</p><h3 className="mt-2 font-semibold text-gray-900 dark:text-gray-100">Bring your profile into ChatGPT, Claude, Gemini, and Cursor.</h3><p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Preview what you share and use the best setup method each provider supports.</p><span className="mt-4 inline-block font-mono text-sm text-emerald-700 dark:text-emerald-400">Open integrations →</span></a><MeOSCTA placement="dashboard" variant="compact" /></div>
  </section>;
}
