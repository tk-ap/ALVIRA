...
import { createEncryptedMeosDossier } from "~/lib/meos-dossier";

export const Route = createFileRoute("/meos")({
  head: () => ({
    meta: [
      { title: "MeOS — Personal Alignment Operating System" },
      {
        name: "description",
        content:
          "Turn your AI profile into a personal operating system for decisions, direction, and daily life.",
      },
    ],
  }),
  component: MeosPage,
...
const outputs = [
  ...
  "Agent-ready MeOS Builder Kit",
  "Optional private hosted MeOS site",
];
const steps = [
      ...
      <main id="main-content">
        ...
        >
          ...
          <h1 className="mt-5 max-w-3xl text-4xl font-semibold leading-tight tracking-tight text-gray-900 dark:text-gray-100 sm:text-6xl">
            Your personal alignment operating system.
          </h1>
          <p className="mt-7 max-w-2xl text-lg leading-8 text-gray-600 dark:text-gray-400">
            MeOS builds a living portrait of who you are — your values,
            patterns, goals, professional history, and sense of purpose — then
            turns it into a private daily companion for clearer personal and
        ...
        <section
          className="border-y border-gray-200 bg-emerald-50/50 dark:border-gray-800 dark:bg-emerald-950/30"
        ...
        >
          <div className="mx-auto max-w-4xl px-6 py-16 text-center">
            ...
            <p className="mt-4 max-w-2xl text-base leading-relaxed text-gray-600 dark:text-gray-400">
              ALVIRA captures what to share with AI. MeOS applies that same
              discovery process to your life — building a private operating
              system for personal and professional alignment.
            </p>
            <p className="mt-8 max-w-2xl rounded-lg border border-system/30 bg-system-soft/30 px-5 py-4 text-sm leading-relaxed text-gray-700 dark:bg-emerald-950/30 dark:text-gray-300">
              ↗{" "}
              ...
              first ($20/mo or $192/yr), then purchase MeOS Build below. MeOS
              Care is optional — add it anytime to keep your portrait evolving.
            </p>
            ...
            <p className="mt-8 max-w-2xl text-sm leading-relaxed text-gray-600 dark:text-gray-400">
              <strong className="text-gray-900 dark:text-gray-100">
                Managed hosting is included with MeOS Care.
              </strong>{" "}
              Your private MeOS site is hosted for you — no setup required.
...
function MeosPage() {
  ...
  const [versions, setVersions] = useState<
    Array<{ version: number; source: string; created_at: string }>
  >([]);
  useEffect(() => {
    (async () => {
      try {
        ...
        const entitlements: string[] = await getEntitlements().catch(() => []);
        setHasCare(entitlements.includes("meos_care"));
        if (
          user.isOwner ||
  ...
  const downloadBuilderKit = async (includeStarter = false) => {
    if (!profile) return;
    const zip = new JSZip();
    ...
    a.download = `${
      profile.topic
        ...
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-|-$/g, "") || "meos"
    }-builder-kit${includeStarter ? "-with-starter" : ""}.zip`;
    a.click();
  ...
  const downloadDossier = () => {
    ...
    if (password.length < 10)
      return setFileStatus("Use a password with at least 10 characters.");
    if (password !== confirmPassword)
      return setFileStatus("The passwords do not match.");
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `${profile.topic.toLowerCase().replace(/[^a-z0-9]+/g, "-") || "meos"}-encrypted-dossier.pdf`;
    a.click();
    URL.revokeObjectURL(url);
    setPassword("");
    setConfirmPassword("");
    setFileStatus(
      "Encrypted dossier downloaded. Keep its password separately.",
    );
  };
  const refreshVersions = async () => {
    if (!profile || !hasCare) return;
    setVersions(
      (await getMeosVersions({
  ...
  const syncNow = async () => {
    if (!profile) return;
    const result = await syncMeosProfile({ data: { profileId: profile.id } });
    setFileStatus(`Care sync saved version ${result.version}.`);
  ...
  return (
    <div className="min-h-dvh">
      ...
      <main id="main-content" className="mx-auto max-w-4xl px-6 py-14">
        ...
        {isPreviewUser && (
          <div className="mb-8 rounded-lg border border-system/40 bg-emerald-50 p-5 text-sm leading-6 text-gray-800 dark:bg-emerald-950/30 dark:text-gray-200">
            <strong>
              This is your MeOS Preview — based on 3 of 12 domains.
            </strong>{" "}
            Upgrade to MeOS Build for your full portrait with purpose
        ...
        {!profile ? (
          <div className="border border-gray-200 p-8 dark:border-gray-800">
            <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">
              Your MeOS is ready to begin.
            </h2>
            <p className="mt-3 text-gray-600 dark:text-gray-400">
              Complete a MeOS interview and your integrated portrait will appear
              here.
            </p>
            <a
              href="/app?offering=meos"
              ...
              {tabs
                ...
                .map((t) => (
                  <button
                    key={t}
                    onClick={() => setTab(t)}
                    className={`px-4 py-3 font-mono text-xs uppercase tracking-wider focus:outline-none focus:ring-2 focus:ring-system ${tab === t ? "border-b-2 border-emerald-500 text-system" : "text-gray-500 hover:text-gray-900 dark:hover:text-gray-200"}`}
                  >
                    {t}
                  </button>
                ))}
                  ...
                  <div className="space-y-9">
                    ...
                    <section className="rounded-lg border border-gray-200 p-5 dark:border-gray-800">
                      ...
                      <p className="mt-2 text-sm">
                        A password-protected PDF for private reading and careful
                        sharing.
                      </p>
                      ...
                      <button
                        ...
                        className="mt-4 rounded-lg bg-system-dark px-5 py-3 font-mono text-sm font-semibold text-white hover:bg-system"
                      >
                    ...
                    <section className="rounded-lg border border-gray-200 p-5 dark:border-gray-800">
                      ...
                      <p className="mt-2 text-sm">
                        Choose the agent brief alone, or add a reusable,
                        local-first starter application.
                      </p>
                      <div className="mt-4 flex flex-wrap gap-3">
                        <button
                          type="button"
                          onClick={() => downloadBuilderKit(false)}
                          className="rounded-lg border border-system px-4 py-2 font-mono text-sm text-system"
                        >
                          Core Builder Kit ↓
                        </button>
                        <button
                          type="button"
                          onClick={() => downloadBuilderKit(true)}
                          className="rounded-lg bg-system-dark px-4 py-2 font-mono text-sm font-semibold text-white"
                        >
                          Builder Kit + Starter App ↓
                        </button>
                      </div>
                    </section>
                    <section className="rounded-lg border border-gray-200 p-5 dark:border-gray-800">
                      <h3 className="font-semibold text-gray-900 dark:text-gray-100">
                        4. Private hosted MeOS
                      </h3>
                      <p className="mt-2 text-sm">
                        Automatically generated from this profile and protected
                        by your ALVIRA sign-in.
                      </p>
                      <a
                        href={`/meos/site/${profile.id}`}
                        className="mt-4 inline-block rounded-lg bg-system-dark px-4 py-2 font-mono text-sm font-semibold text-white"
                      >
                        Open hosted MeOS →
                      </a>
                    </section>
                    <section className="rounded-lg border border-gray-200 p-5 dark:border-gray-800">
                      <h3 className="font-semibold text-gray-900 dark:text-gray-100">
                        5. Care synchronization
                      </h3>
                      {hasCare ? (
                        <>
                          <p className="mt-2 text-sm">
                            Save a restorable snapshot whenever your portrait
                            reaches a meaningful point.
                          </p>
                          <div className="mt-4 flex flex-wrap gap-3">
                            <button
                              type="button"
                          ...
                          {versions.length > 0 && (
                            <div className="mt-5 space-y-2">
                              {versions.map((v) => (
                                <div
                                  key={v.version}
                                  className="flex items-center justify-between gap-4 border-t border-gray-200 pt-2 text-sm dark:border-gray-800"
                                >
                                  <span>
                                    Version {v.version} ·{" "}
                                  ...
                                  <button
                                    ...
                                    className="font-mono text-xs text-system"
                                  >
                                    Restore
                                  </button>
                                </div>
                        ...
                        <p className="mt-2 text-sm">
                          MeOS Care adds version history, restorable snapshots,
                          and ongoing managed updates.{" "}
                          <a
                            href="/meos#pricing-heading"
                            className="text-system underline"
                          >
                            Add MeOS Care →
                          </a>
                        </p>
                    ...
                    {fileStatus && (
                      <p
                        role="status"
                        className="rounded-md bg-emerald-50 px-4 py-3 text-sm text-system-dark dark:bg-emerald-950/30 dark:text-system"
                      >
                        {fileStatus}
                      </p>
                    )}
